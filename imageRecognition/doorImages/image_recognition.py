...
import keras
from keras.layers import Dense, Activation, Conv2D, MaxPool2D, GlobalAvgPool2D, BatchNormalization, add, Input, Flatten
...
save_direc = os.path.join(os.getcwd(), 'models')
...
def ResNet50(input_shape, num_classes=3):
...
def train_network():
    ...
    model = ResNet50((im_length, im_width, 3), num_classes=num_classes)
    ...
    train_generator = train_datagen.flow_from_directory(DATASET_TRAIN_DIR, target_size=(im_length, im_width),
                                                        batch_size=batch_size, class_mode="categorical")
    test_generator = test_datagen.flow_from_directory(DATASET_TEST_DIR, target_size=(im_length, im_width), batch_size=batch_size,
                                                      class_mode="categorical")

    model.fit_generator(train_generator, steps_per_epoch=int(60 / batch_size), epochs=epochs,
                        validation_data=test_generator,
                        validation_steps=int(15 / batch_size), callbacks=[checkpoint, lr_scheduler])
    # ----------------- The Section Responsible for Inference ---------------------
CLASS_INDEX = None

MODEL_PATH = os.path.join(execution_path, "idenprof_weight_model.008-0.7272727326913313.h5")
...
def run_inference():
    model = ResNet50(input_shape=(im_length, im_width, 3), num_classes=3)
    model.load_weights(MODEL_PATH)

    picture = os.path.join(execution_path, "9043-4.jpg")
